class_name SkeletonMinionAnimator extends RefCounted

var _animator: EnemyAnimator
var _params: SkeletonMinionAnimationParams

func _init(animator: EnemyAnimator, params: SkeletonMinionAnimationParams) -> void:
	_animator = animator
	_params = params

func blend_loco_timescale(value: float) -> void:
	_animator.set_param(_params.loco_time_scale, value)

func blend_loco_move(value: float) -> void:
	_animator.set_param(_params.loco_blend_position, value)

func punch_attack() -> void:
	_animator.oneshot_fire(_params.oneshot_punch)

func hit_strong() -> void:
	_animator.oneshot_fire(_params.oneshot_hit_strong)

func hit_weak() -> void:
	_animator.oneshot_fire(_params.oneshot_hit_weak)

func die() -> void:
	print("play die anim, by travel to: ", _params.state_death)
	_animator.playback_travel(EnemyAnimator.StatePlayback.FullBody, _params.state_death)
