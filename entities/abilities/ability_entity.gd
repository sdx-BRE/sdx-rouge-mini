...
func setup_character_ability(data: CharacterAbilityEffect, context: CharacterAbilityExecuteContext) -> void:
	if enable_debug:
...
func launch_character_ability(data: CharacterAbilityEffect, context: CharacterAbilityExecuteContext) -> void:
	if enable_debug:
		var err := DbgHelper.err("AbilityEntity.setup_character_ability", "must be overwritten by child implementations")
		push_error(err, " - data: ", data, ", context: ", context)
