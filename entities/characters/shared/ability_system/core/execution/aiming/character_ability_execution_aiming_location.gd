class_name CharacterAbilityExecutionAimingLocation extends CharacterAbilityExecutionAimingBase
...
func handle_input(event: InputEvent) -> bool:
	...
	if event.is_action_pressed(_data.input_trigger):
		_cleanup_ui_state()
		_emit_target_aquired(CharacterAbilityAimingResultLocation.new(_aim_pos))
		return true
