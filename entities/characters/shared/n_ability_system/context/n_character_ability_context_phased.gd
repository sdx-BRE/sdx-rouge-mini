class_name NCharacterAbilityContextPhased extends NCharacterAbilityContext
...
func animate(data: NCharacterAbilityExecutionPhased) -> void:
	_anim_tree.set(data.anim.trigger + "/request", AnimationNodeOneShot.ONE_SHOT_REQUEST_FIRE)

func unproject_position(position: Vector3) -> Vector2:
	return _viewport.get_camera_3d().unproject_position(position)

func get_animation_position(data: NCharacterAbilityExecutionPhased) -> float:
	var value = _anim_tree.get(data.anim.trigger + "/current_position")
	
	return 0.0 if value == null else value

func update_cast_point(data: NCharacterAbilityExecutionPhased) -> void:
